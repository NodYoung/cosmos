...
int main(int argc, char** argv)
{
  ...
  }
  {
    ...
    for (int i=0; i<T_gripper2base.size(); i++) {
      std::cout << T_gripper2base.at(i) << std::endl;
    }
    std::cout << "R_target2cam" << std::endl;
    for (int i=0; i<R_target2cam.size(); i++) {
      std::cout << R_target2cam.at(i) << std::endl;
    }
    std::cout << "T_target2cam" << std::endl;
    for (int i=0; i<T_target2cam.size(); i++) {
      std::cout << T_target2cam.at(i) << std::endl;
    }
  }
  ...
  LOG(INFO) << Homo_cam2gripper;
  LOG(INFO) << "Homo_cam2gripper 是否包含旋转矩阵：" << isRotatedMatrix(Homo_cam2gripper);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  **************************************************/
  ...
  LOG(INFO) << "1 : " << Homo_gripper2base[0] * Homo_cam2gripper * Homo_target2cam[0] << endl;
  LOG(INFO) << "2 : " << Homo_gripper2base[1] * Homo_cam2gripper * Homo_target2cam[1] << endl;
  //标定板在相机中的位姿
  LOG(INFO) << "3 : " << Homo_target2cam[1] << endl;
  LOG(INFO) << "4 : " << Homo_cam2gripper.inv() * Homo_gripper2base[1].inv() * Homo_gripper2base[0] * Homo_cam2gripper * Homo_target2cam[0] << endl;

  LOG(INFO) << "----手眼系统测试-----" << endl;
  LOG(INFO) << "机械臂下标定板XYZ为：" << endl;
